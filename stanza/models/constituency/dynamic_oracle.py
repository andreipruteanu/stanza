...
def fix_close_shift_shift(gold_transition, pred_transition, gold_sequence, gold_index, root_labels):
    ...
    end_index = find_constituent_end(gold_sequence, gold_index+1)
    ...
    # it is not clear if youw want to close at the end of
    # stuff_2 or build stuff_3 instead.
    if not isinstance(gold_sequence[end_index], CloseConstituent):
        return None

    return gold_sequence[:gold_index] + gold_sequence[gold_index+1:end_index] + [CloseConstituent()] + gold_sequence[end_index:]
...
class RepairType(Enum):
    ...
    # If the model is supposed to build a block after
    # a Close operation, attach that block to the
    # piece to the left
